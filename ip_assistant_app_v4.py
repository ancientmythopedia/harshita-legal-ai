...
st.set_page_config(page_title="IP Assistant Prototype (v4)", layout="wide")
st.title("IP Assistant Prototype (v4)")
...
def highlight_keyword_col(col_series):
    # Highlight the MatchedKeyword cells only
...
if (portfolio is not None) and filings_file:
    ...
    if len(alerts_df):
        ...
        # Styling: row shading + MatchedKeyword cell highlight
        styled = filt.style.apply(base_row_style, axis=1).apply(highlight_keyword_col, subset=["MatchedKeyword"])
        st.dataframe(styled, use_container_width=True)

        st.download_button("Download alerts (CSV)", data=filt.to_csv(index=False), file_name="tm_watch_alerts_filtered.csv", mime="text/csv")
        st.caption("Row highlighting by Similarity; MatchedKeyword column highlighted. Use filters above to narrow results.")
    else:
...
st.divider()
st.caption("Prototype only. Use official IP data sources & legal review before action. Highlighting: >=0.90 red, >=0.80 orange, >=0.70 yellow. MatchedKeyword cells highlighted; filters + search added.")
