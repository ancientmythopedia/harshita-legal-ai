...
st.set_page_config(page_title="IP Assistant Prototype (v2)", layout="wide")
st.title("⚖️ IP Assistant Prototype (v2)")
...
sim_threshold = st.sidebar.slider("Similarity threshold for watch", 0.0, 1.0, 0.82, 0.01)

# ---------- 1) Portfolio & Renewals ----------
st.header("1) Upload IP Portfolio")
portfolio_file = st.file_uploader("Upload ip_portfolio_template.xlsx", type=["xlsx"], key="portfolio")
    ...
    if "RenewalDate" in portfolio.columns:
        ...
        if len(due):
            ...
            for _, r in due.iterrows():
                msg = f"""Subject: Renewal reminder – {r.get('Trademark','')} (Class {r.get('Class','')}) due {r.get('RenewalDate','')}

Hello {r.get('Owner','')},

This is a friendly reminder that your trademark "{r.get('Trademark','')}" (Class {r.get('Class','')}, Reg. No. {r.get('RegNo','')}) is due for renewal on {r.get('RenewalDate','')}.
...
"""
            ...
            with st.expander("Optional: Send reminder emails now (SMTP)"):
                ...
                sel = st.multiselect("Select marks to email", options=list(due.index), format_func=lambda i: f"{due.loc[i,'Trademark']} (due {due.loc[i,'RenewalDate']})")
...
filings_file = st.file_uploader("Upload new filings CSV (sample: new_tm_filings_sample.csv)", type=["csv"], key="filings")
...
if portfolio is not None and filings_file:
    ...
    if len(alerts_df):
        st.dataframe(alerts_df.sort_values("Similarity", ascending=False), use_container_width=True)
        st.download_button("⚠️ Download alerts (CSV)", data=alerts_df.to_csv(index=False), file_name="tm_watch_alerts.csv", mime="text/csv")
    else:
        st.info("No potential conflicts found at the current threshold.")
...
with st.form("license_form_v2"):
    ...
    if submitted:
        ...
        bio = BytesIO()
        doc.save(bio)
        st.download_button("📄 Download License Agreement", data=bio.getvalue(), file_name="Trademark_License_Agreement.docx", mime="application/vnd.openxmlformats-officedocument.wordprocessingml.document")

st.divider()
st.caption("Prototype only. Use official IP data sources & legal review before action. For better matching, install 'rapidfuzz'. For email sending, use app passwords or trusted SMTP.")
